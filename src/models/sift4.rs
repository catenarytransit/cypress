...
thread_local! {
    static OFFSET_ARR: RefCell<Vec<SiftOffset>> = RefCell::new(Vec::with_capacity(32));
}

pub fn sift4(s1: &[u8], s2: &[u8], max_offset: usize, max_distance: usize) -> usize {
    OFFSET_ARR.with(|arr_cell| {
        let mut offset_arr = arr_cell.borrow_mut();
        offset_arr.clear();
            ...
            if s1[c1] == s2[c2] {
                ...
                while i < offset_arr.len() {
                    ...
                    if c1 <= ofs.c1 || c2 <= ofs.c2 {
                        ...
                        if is_trans {
                            trans += 1;
                        } else if !offset_arr[i].trans {
                            offset_arr[i].trans = true;
                            trans += 1;
                        }
                        break;
                    } else if c1 > ofs.c2 && c2 > ofs.c1 {
                        offset_arr.swap_remove(i);
                    } else {
                        i += 1;
                    }
                }
